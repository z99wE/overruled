      ...
      <div className="relative w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl text-slate-900">
        <div className="flex items-center justify-between pb-4 border-b border-slate-100">
          <h3 className="font-display text-xl font-extrabold flex items-center gap-2">
            <span>🛡️</span>
            <span>Zero-Leak Audit & Legal Standards</span>
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="my-4 max-h-[60vh] space-y-4 overflow-y-auto text-xs sm:text-sm text-slate-600 pr-2">
          <div className="rounded-xl bg-blue-50 p-3 border border-blue-200">
            ...
            <p>Your contract drafts and privileged notes are processed entirely in-browser. We never retain, store, or train AI models on your confidential legal documents.</p>
          </div>
          ...
          <div className="rounded-xl bg-amber-50 p-3 border border-amber-200">
            ...
            <p>Every citation and legal ratio is certified against official court reports across US, UK, Australia, Canada, European Union, South Africa, and India.</p>
          </div>
          ...
          <div className="rounded-xl bg-emerald-50 p-3 border border-emerald-200">
            ...
            <p>Identifies one-sided indemnities, uncapped consequential liabilities, non-compete overreach, and jurisdiction traps with actionable reciprocal redlines.</p>
          </div>
          ...
          <div className="rounded-xl bg-purple-50 p-3 border border-purple-200">
            ...
            <p>Test litigation arguments and contract defense motions against strict simulated benches to verify enforceability before stepping into court.</p>
          </div>
        </div>
  ...
  const handleDraw = () => {
    ...
    setTimeout(() => {
      const precedents = [
        ...
        '🏛️ Mabo v. Queensland (1992) · Proprietary Title & Extinguishment Limits',
        '⚖️ Donoghue v. Stevenson (1932) · Neighbor Principle & Duty of Care',
        '🔒 Schrems II (2020) · Cross-Border Data Transfer Safeguards',
        '🛡️ Carlill v. Carbolic Smoke Ball (1893) · Unilateral Contracts & Consideration',
      ];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-md">
      <div className="relative w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl text-slate-900 text-center">
        <div className="flex items-center justify-between pb-3">
          <h3 className="font-display text-xl font-extrabold flex items-center gap-2">
            <span>🏛️</span>
            <span>Precedent Discovery Vault</span>
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="my-6 flex flex-col items-center">
          <div className={`flex h-28 w-28 items-center justify-center rounded-3xl border-4 border-amber-400 bg-amber-50 text-5xl shadow-lg transition-transform ${drawing ? 'rotate-180 scale-110 duration-500' : ''}`}>
            {drawing ? '⚖️' : result ? '📜' : '🏛️'}
          </div>

          {result ? (
            <div className="mt-4 rounded-xl bg-blue-50 p-3.5 border border-blue-200 font-sans text-xs sm:text-sm font-extrabold text-blue-950 text-left anim-pop">
              {result}
            </div>
          ) : (
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-md">
      <div className="relative w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl text-slate-900">
        <div className="flex items-center justify-between pb-4 border-b border-slate-100">
          <h3 className="font-display text-xl font-extrabold flex items-center gap-2">
            <span>⚖️</span>
            <span>Counsel Workspace & Dockets</span>
          </h3>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="my-5 space-y-4">
          <div className="flex items-center justify-between rounded-2xl bg-amber-50 p-4 border border-amber-200">
            <div>
              <div className="text-[11px] font-bold text-amber-800 uppercase tracking-tight">Available Bench Favor</div>
              <div className="font-display text-3xl font-black text-amber-950">{chips} <span className="text-sm font-sans font-bold">chips</span></div>
            </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs py-1.5 border-b border-slate-100">
              <span className="text-slate-500">Certified Precedents Loaded</span>
              <span className="font-mono font-bold text-slate-800">59 Decisions</span>
            </div>
            <div className="flex items-center justify-between text-xs py-1.5 border-b border-slate-100">
              <span className="text-slate-500">Privacy Status</span>
              <span className="font-mono font-bold text-emerald-600">Zero-Leak Local</span>
            </div>
            <div className="flex items-center justify-between text-xs py-1.5 border-b border-slate-100">
              <span className="text-slate-500">Jurisdictions</span>
              <span className="font-mono font-bold text-slate-800">7 Common Law</span>
            </div>
          </div>
        </div>
        ...
        <div className="pt-3 border-t border-slate-100 flex justify-end">
          <button
            type="button"
            onClick={onClose}
            className="rounded-full bg-slate-900 px-6 py-2.5 font-sans text-xs font-bold text-white hover:bg-slate-800 transition-colors cursor-pointer"
          >
            Close
          </button>
        </div>
