...
export function ShopModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-md font-sans selection:bg-amber-200 selection:text-slate-950">
      <div className="w-full max-w-2xl rounded-3xl border border-slate-200/90 bg-white shadow-2xl overflow-hidden">
        <header className="flex items-center justify-between border-b border-slate-100 bg-slate-50/90 px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            <span className="rounded-full border border-amber-300 bg-amber-50 px-4 py-1 font-sans text-xs font-black text-amber-950 shadow-2xs">
              Chips: {chips}
            </span>
            <button
        ...
        <div className="grid max-h-[65vh] gap-3.5 overflow-y-auto p-6 sm:grid-cols-2">
          {JOKERS.map((j) => {
            ...
            return (
              <div
                key={j.id}
                className="flex flex-col justify-between rounded-2xl border border-slate-200 bg-slate-50/70 p-4 shadow-2xs transition-all hover:bg-white hover:border-blue-300 hover:shadow-xs"
              >
                <div>
                  <div className="mb-1.5 flex items-center justify-between gap-2">
                    <p className="font-display text-sm font-extrabold text-slate-900">{j.name}</p>
                    <span className={`rounded-full border px-2.5 py-0.5 font-sans text-[10px] font-bold ${rarityCls(j.rarity)}`}>
                      {j.rarity}
                    </span>
                  </div>
                ...
                >
                  {isOwned ? 'Admitted to Deck' : `Unlock for ${j.cost} Chips`}
                </button>
              </div>
            );
          })}
